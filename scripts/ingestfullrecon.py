...
def ensure_source_and_locations(cur):
    ...
    locations = [
        ...
        ("MTL", "Montreal Distribution Centre", "QC", "CA", {"source": "inventoy.xlsx", "sheet": "MTL"}),
        ("VAN", "Vancouver Distribution Centre", "BC", "CA", {"source": "inventoy.xlsx", "sheet": "VAN"}),
        ("EDM", "Edmonton Distribution Centre", "AB", "CA", {"source": "inventoy.xlsx", "sheet": "EDM"}),
    ]
    ids = {}
    ...
    for sheet_name in ["MTL", "VAN", "EDM"]:
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            product_id = upsert_product(
                ...
                "Uncategorized",
                "Each",
                {"source": "inventoy.xlsx", "sheet": sheet_name, "row": rd},
            )
            cur.execute(
                """
                ...
                (
                    ...
                    json.dumps({"source": "inventoy.xlsx", "sheet": sheet_name, "row": rd}, default=str),
                ),
            )
            counts["inventory"] += 1
            pending += 1
